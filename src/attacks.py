...
def fgsm_attack(model, images, labels, criterion, epsilon, use_amp=False):
    model.eval()
...
def multi_step_fgsm_attack(model, images, labels, criterion, epsilon, alpha, steps, use_amp=False):
    model.eval()
...
def untargeted_pgd_attack(model, images, labels, criterion, epsilon, alpha, steps, random_start=True, use_amp=False):
    model.eval()
    clean_images = images.clone().detach()
    if random_start:
...
def targeted_pgd_attack(model, images, target_labels, criterion, epsilon, alpha, steps, use_amp=False):
    model.eval()
    clean_images = images.clone().detach()
    adv_images = clean_images.clone().detach()
