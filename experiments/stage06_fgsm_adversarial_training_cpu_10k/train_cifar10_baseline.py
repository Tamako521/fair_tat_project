...
def train_one_epoch_fgsm_at(model, trainloader, criterion, optimizer, device, args):
    model.train()
    total_loss = 0.0
    ...
    for images, labels in trainloader:
        ...
        adv_images = fgsm_attack(model, images, labels, criterion, args.epsilon)
...
def train_one_epoch_targeted_pgd_at(model, trainloader, criterion, optimizer, device, args):
    model.train()
    total_loss = 0.0
    total_clean_loss = 0.0
    total_adv_loss = 0.0
    ...
    for images, labels in trainloader:
        ...
        targeted_adv_images = targeted_pgd_attack(
            model,
            images,
            target_labels,
            criterion,
        ...
        model.train()
        clean_outputs = model(images)
        adv_outputs = model(targeted_adv_images)
        clean_loss = criterion(clean_outputs, labels)
        adv_loss = criterion(adv_outputs, labels)
        loss = (1 - args.adv_weight) * clean_loss + args.adv_weight * adv_loss
...
def parse_args():
    ...
    parser.add_argument("--training-mode", type=str, default="clean", choices=["clean", "fgsm_at", "targeted_pgd_at"])
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        if args.training_mode == "fgsm_at":
            train_metrics = train_one_epoch_fgsm_at(model, trainloader, criterion, optimizer, device, args)
        elif args.training_mode == "targeted_pgd_at":
            train_metrics = train_one_epoch_targeted_pgd_at(model, trainloader, criterion, optimizer, device, args)
        else:
        ...
        if args.training_mode in {"fgsm_at", "targeted_pgd_at"}:
        ...
        if args.training_mode in {"fgsm_at", "targeted_pgd_at"}:
            message += (
                f" | train clean loss: {epoch_metrics['train_clean_loss']:.4f}"
                f" | train adv loss: {epoch_metrics['train_adv_loss']:.4f}"
            )
